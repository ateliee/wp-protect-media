...
<?php if(!$this->is_allow_override()){ ?>
    <div class="notice"><p><?= $this->is_allow_override(); ?>php.iniのAllowOverrideが有効か確認してください。</p></div>
<?php } ?>
<?php if(file_exists(self::HTACCESS_PATH) && !is_writable(self::HTACCESS_PATH)){ ?>
    <?php if($is_error){ ?>
        <div class="notice notice-alt notice-error"><p>.htaccessへの書き込み権限がありません。下記を<?= self::HTACCESS_PATH; ?>に記載ください</p></div>
        <div class="notice notice-error">
        <pre><?= $this->get_htaccess_str($path); ?></pre>
        </div>
    <?php }else{ ?>
...
<?php if($message = get_transient(self::PLUGIN_SETTING_TRANSIENT_MESSAGE)){ ?>
    <div id="message" class="notice notice-success"><p><?= $message; ?></p></div>
<?php } ?>
            <form action="" method='post' id="my-submenu-form">
                <?php wp_nonce_field(self::CREDENTIAL_ACTION, self::CREDENTIAL_NAME) ?>
                <p>
                    <label for="title">パス設定</label>
                    <input type="text" name="path" value="<?= $path ?>" placeholder="uploadsからのパスを指定してください" size="60" />
                </p>
                <p>
